...
<template>
  <div class="flex justify-end gap-3 mt-8">
    <button
      v-if="!isFirst"
      @click="$emit('previous')"
      class="min-w-[120px] border border-primary-500 bg-white text-primary-500
             hover:bg-gray-50 rounded-lg px-5 py-2 flex items-center gap-2 transition"
    >
      <VsxIcon iconName="ArrowLeft" :size="18" />
      {{ prevText || "Previous" }}
    </button>
    <button
      v-if="!isLast"
      @click="$emit('next')"
      class="min-w-[120px] bg-primary-500 hover:bg-primary-600 text-white
    ...
    <button
      v-if="isLast"
      @click="$emit('finish')"
      class="min-w-[120px] bg-primary-600 hover:bg-primary-700 text-white
