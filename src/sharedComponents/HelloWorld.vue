...
<template>
  <div class="p-6 w-full h-full bg-gray-100">
    ...
    <!-- Success Dialog -->
    <StatusDialog v-model:visible="successDialog" :icon="successIcon" title="User Created Successfully"
      description="New user account has been added. Please wait… you will be redirected shortly." :showSpinner="true" />

    <!-- Danger Dialog -->
    <StatusDialog v-model:visible="dangerDialog" :icon="dangerIcon"
      title="Validation failed. Please check and try again!" :showSpinner="true" />

    <!-- Delete Dialog -->
    <StatusDialog v-model:visible="deleteDialog" :icon="alertIcon" title="Are you sure you want to delete this user?"
      :buttons="[
        { label: 'Cancel', variant: 'ghost', action: 'cancel', },
        { label: 'Yes, Delete', variant: 'danger', action: 'confirm' }
      ]" @confirm="handleDeleteConfirm" />

    <StatusDialog v-model:visible="logoutDialog" :icon="timerIcon" title="Your session will expire in"
      timer="05:00 minutes" :buttons="[
        { label: 'Stay Logged in ', variant: 'ghost', action: 'cancel' },
        { label: 'Log out now', variant: 'primary', action: 'confirm' }
      ]" @confirm="handleDeleteConfirm" />

    <!-- Download Dialog -->
    <StatusDialog v-model:visible="downloadDialog" :icon="downloadIcon" title="Download Template"
      description="You are about to download the Excel template (.xlsx) for bulk user uploads." :loading="isDownloading"
      :buttons="[
