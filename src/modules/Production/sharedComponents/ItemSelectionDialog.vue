...
const columns = computed(() => {
  return [
    { field: 'itemCode', header: t('itemDialog.itemCode'), sortable: true },
    { field: 'itemName', header: t('itemDialog.itemName'), sortable: true },
...
<template>
  <Dialog v-model:visible="isVisible" modal :header="t('itemDialog.selectItem')" :style="{ width: '60vw' }"
    :breakpoints="{ '960px': '75vw', '641px': '90vw' }">
    <div class="flex flex-col gap-4">
      <span class="w-full">
        <InputText v-model="searchQuery" :placeholder="t('itemDialog.search')"
          class="w-full bg-gray-50 border-gray-200" />
      </span>

      <div class="overflow-hidden rounded-lg border border-gray-100">
        <DynamicTable :columns="columns" :data="filteredItems" :paginator="true" :rows="5" :showView="false"
          ...
          <template #col-action="{ data }">
            <div class="flex justify-end">
              <BaseButton :label="t('itemDialog.add')" size="small" varient="primary" @click="selectItem(data)" />
            </div>
          </template>
        </DynamicTable>
      </div>
