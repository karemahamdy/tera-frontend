...
<template>
    <div class="p-6 w-full h-full bg-gray-100">
        <Dialog v-model:visible="isVisible" modal :style="{ width: '80%' }">
            <div>
                <PageHeader title="PhysicalCount.serialItemSelection"
                    :subtitle="`${selectedItem?.itemName ?? ''} (${selectedItem?.code ?? ''})`" :showSearch="false" />
        ...
        <StatusDialog v-model:visible="showReleaseDialog" :icon="alertIcon" :title="confirmationTitle" :buttons="[
